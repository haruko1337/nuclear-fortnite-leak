...
public:
	...
	static inline TUObjectArray GetGlobalObjects()
	{
		...
		if (!GObjects)
			GObjects = (FUObjectArray*)((DWORD64)GetModuleHandleW(NULL) + 0x968B288);
...
namespace G
{
	...
	float CornerBoxThicc = 1.0f;
	float RedDistance = 50.0f;
	float CornerBoxScale = 0.20;
	bool PunktierSnaplines = false;
	float PunktierPower = 20.0f;
	...
	bool ShowTimeConsumed = false;
	float Smooth = 0.5;
	bool CollisionDisableOnAimbotKey = false;
	...
	int MovSpeedhackScale = 3;
	bool AimbotEnable = false;
	bool EspSentries = false;
	bool EspZombies = false;
	bool EspPlayers = false;
	bool EspCorpses = false;
	bool EspAnimals = false;
	bool EspDrones = false;
	float RenderDist = 50;
	float ChestsRdist = 50;
	UObject* Closest;
	bool AimbotTargetPlayers = false;
	bool AimbotTargetZombies;
	float LootRenderDist = 50;
	int IconScale = 21;
...
inline FVector xWorldToScreen(FVector worldloc, FRotator camrot)
{
	...
	if (vTransformed.Z < 1.f || mytan(fov_angle * (float)M_PI / 40.f) == 0.f) return FVector(0, 0, 0);

	screenloc.X = ScreenCenterX + vTransformed.X * (ScreenCenterX / mytan(fov_angle * (float)M_PI / 40.f)) / vTransformed.Z;
	screenloc.Y = ScreenCenterY - vTransformed.Y * (ScreenCenterX / mytan(fov_angle * (float)M_PI / 40.f)) / vTransformed.Z;
...
bool xWorldToScreen(FVector WorldLocation, FVector2D& outLocScreen)
{
	...
	float ScreenCenterX = static_cast<float>(g_ScreenWidth) / 2.0f;
	float ScreenCenterY = static_cast<float>(g_ScreenHeight) / 2.0f;
	auto f = (ScreenCenterX / mytan(FovAngle * M_PI / 60.f));
			...
			for (int i = 0; i < actors.Num(); i++)
			{
				...
				else if (levelIndex == 0 && Object_IsA(actor, SC_FortPlayerPawn()))
				{
					...
					if (W2S(loc, sp))
					{
						...
						{
							xDrawText(E(L"ENEMY IS CLOSE!!"), { (float)(g_ScreenWidth / 2) , (float)(g_ScreenHeight - 180) }, Colors::Red);
							bEnemyClose = true;
						}
						...
						{
							closestEnemyAss = GetBone3D(mesh, Bones::Spine_03);
				...
				else if (Object_IsA(actor, projectiles_class))
				{
					...
					{
						FHitResult xxx;
						K2_SetActorLocation(actor, closestEnemyAss, true, true, &xxx);
					}
				}
...
void HkPostRender(UObject* _this, UObject* canvas)
{
	...
	if (GCanvas && G::ShowTimeConsumed)
	{
		...
		sprintf(time_buff, E("PostRender: time played: %d"), delta);
...
void MenuDrawTabs()
{
	...
	if (g_Menu)
	sprintf(logo_buff, E("NUCLEAR FIXED LEAKED BY Kynch#9667"), E(__DATE__), E(__TIME__, g_Menu));
...
bool __stdcall DllMain(void* hModule, unsigned long ul_reason_for_call, void* lpReserved)
{
	if (ul_reason_for_call == 1)
	{
		...
		//NiggerPE(hModule);
		MessageBoxA(0, "Preparations", "Preparations Success", 0);

		dprintf(E("Preparations are done, calling main()"));

		main();
		MessageBoxA(0, "main", "main Success", 0);
	...
	{
		MessageBoxA(0, "Failed injecting", "Crashed", 0);
		exit(EXIT_FAILURE);
	}
	{
